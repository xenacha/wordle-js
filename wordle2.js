...
function appStart() {
  const nextLine = () => {
    if (attempts === 6) return gameover(); //6번까지만 시도 가능 -> attemps=6 : 7번째 -> nextLine X
    attempts += 1; //다음 줄
    index = 0; // 칸 순서 초기화 -> 첫째 칸부터
  };

  const gameover = () => {
    window.removeEventListener("keydowmn", handleKeyDown); //게임 종료 시 키보드 입력 X
  };
  ...
  const handleEnterKey = () => {
    for (let i = 0; i < 5; i++) {
      ...
      if (입력한_글자 === "") {
        block.style.backgroundColor = "gray"; // 또는 아무 처리도 안 함
    ...
    /* if ((맞은_갯수 = 5)) gameover();
    else nextLine(); // 5개 다 맞으면 게임종료(키보드 이벤트 안먹어야 함), 틀리면 다음 줄
 */
 ...
 */
    if (event.key === "Enter") {
      ...
      //index-> 몇 번째 칸인지 추적
      return; // 5글자(index 04까지) 넘으면 추가입력 막음
